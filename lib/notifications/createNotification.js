...
export async function createNotification({
  ...
  const { data: notification, error: notificationError } =
    await supabaseAdmin
      .from("notifications")
      .insert({
        ...(eventKey ? { id: createHash("sha256").update(eventKey).digest("hex").slice(0,32).replace(/(.{8})(.{4})(.{4})(.{4})(.{12})/, "$1-$2-$3-$4-$5") } : {}),
        user_id: userId,
        post_id: postId,
        type,
        message,
  ...
  if (notificationError?.code === "23505") return { smsSent: false, smsReason: "Already notified." };
    ...
    return {
      ...
      smsReason:
        error instanceof TwilioConfigurationError
          ? "SMS notifications are not configured."
