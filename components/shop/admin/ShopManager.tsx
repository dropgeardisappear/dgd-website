...
function OrderCard({ order, onSaved }: { order: ShopOrder; onSaved: () => void }) {
  ...
  return <article className={s.panel}>
    ...
    {order.shipping_details?.method === "stamped_letters" && <div className={s.notice}><strong>Pack {order.shipping_details.envelope_count} stamped {order.shipping_details.envelope_count === 1 ? "envelope" : "envelopes"}</strong><ul>{order.shipping_details.packing.map(item => <li key={item.product_id}>{item.title}: {item.quantity} total, up to {item.items_per_envelope} per envelope ({item.envelopes} {item.envelopes === 1 ? "envelope" : "envelopes"}).</li>)}</ul><p>Keep each envelope flexible, evenly filled, no thicker than ¼ inch, and under 1 oz. Weigh before adding postage. No package tracking is included.</p></div>}
